...
import { Loader2, Mail, ArrowUpRight, Eye, Reply, Send } from 'lucide-react';
...
export function EmailActivityTab() {
  ...
  const fetchLogs = async () => {
    ...
    const { data: emailLogs } = await supabase
      .from('email_logs')
      .select('id, subject, status, sent_at, provider, lead_id, sequence_id')
    ...
    if (leadIds.length > 0) {
      const { data: leads } = await supabase
        ...
        .in('id', leadIds);
      if (leads) {
        leads.forEach((l: any) => { leadMap[l.id] = `${l.first_name} ${l.last_name}`; });
      }
    }
    ...
    if (sequenceIds.length > 0) {
      const { data: seqs } = await supabase
        ...
        .in('id', sequenceIds);
      if (seqs) {
        (seqs as any[]).forEach((s) => { seqMap[s.id] = s.name; });
      }
    }
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Track all emails sent from the platform</p>
      </div>
